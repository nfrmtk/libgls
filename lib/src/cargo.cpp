...
Response gls::Cargo::Get(std::string ordering, std::int64_t page,
...
Response Cargo::GetById(std::int64_t id) {
  auto response = cpr::Get(
      cpr::Url{auth.GetBaseUrl() + "cargo/"s + std::to_string(id) + '/'},
      cpr::Bearer{auth.GetCredentials()->access_token});
  auto j = nlohmann::json::parse(response.text);
  return std::make_pair(std::move(j), response.status_code);
}
Response Cargo::ChangeById(std::int64_t id, const models::TCargo& replacement) {
  ...
  auto response = cpr::Put(
      cpr::Url{auth.GetBaseUrl() + "cargo/"s + std::to_string(id) + '/'},
      cpr::Bearer{auth.GetCredentials()->access_token}, cpr::Header{headers},
      cpr::Body{to_string(body)});
  auto j = nlohmann::json::parse(response.text);

  return std::make_pair(std::move(j), response.status_code);
}
std::int64_t Cargo::Delete(std::int64_t id) {
  auto params = cpr::Parameters{{"id", std::to_string(id)}};
  auto response = cpr::Delete(
      cpr::Url{auth.GetBaseUrl() + "cargo/"s + std::to_string(id) + '/'},
      std::move(params), cpr::Bearer{auth.GetCredentials()->access_token});
