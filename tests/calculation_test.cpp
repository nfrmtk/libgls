...
TEST_CASE("calculation") {
  ...
  CalculationFixture calcfixture(std::make_unique<ProjectFixture>(project, "calculation-ci"),
                                 std::make_unique<ContainerFixture>(container, std::move(object)),
                                 calc);
  ...
  SECTION("basic") {
    ...
    REQUIRE(std::find_if(rezs.begin(),
                         rezs.end(),
